...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (authError || !user) {
      console.error('Auth error in toggle:', authError);
      return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 });
    }
    ...
    // Resolved feed_id
    console.log(`Resolved feed_id: ${feed?.id} for url: ${feedUrl}`);
    ...
    if (checkError && checkError.code !== 'PGRST116') { // PGRST116 is "no rows"
      console.error('Error checking state:', checkError);
    }

    const nextSavedState = !existingState?.is_saved;
    ...
    const { error: stateError } = await supabase
      ...
      .upsert({
        ...
        saved_at: nextSavedState ? new Date().toISOString() : null,
        updated_at: new Date().toISOString()
      }, { onConflict: 'user_id, item_id' });

    if (stateError) {
      console.error('Error toggling state:', stateError);
      return new Response(JSON.stringify({ error: 'Failed to toggle bookmark' }), { status: 500 });
    }

    return new Response(JSON.stringify({ is_saved: nextSavedState }), { status: 200 });
  } catch (error) {
